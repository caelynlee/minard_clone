...
m.drawcountries()
m.drawrivers()
m.drawparallels(range(54, 58), labels = [True, False, False, False])  #labels參數的四個布林值分別對應左、右、上、下四個邊界，決定是否在該邊界上顯示緯度標籤
...
for xi, yi, city_name in zip(x, y, city_names):
    ...
    #zorder參數用於控制繪圖元素的疊放順序，數值越大表示該元素會被繪製在其他元素之上
...
for i in range(rows - 1): 
    ...
    m.plot(start_stop_lons, start_stop_lats, color = line_color,linewidth = line_width / 10000, zorder = 1) #原始資料軍隊人數較大才除以1萬
...
for lont, temp_c, annotation in zip(lonts, temp_celsius, annotations):
    axes[1].annotate(text = annotation, xy = (lont - 0.3, temp_c - 7), fontsize = 16)
...
axes[1].set_title("Napolean's disastrous Russian campaign of 1812", loc = "left", fontsize = 30)
plt.tight_layout()
